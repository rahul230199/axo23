...
import { LayoutDashboard, FileText, Package, User, LogOut, ChevronLeft, ChevronRight } from 'lucide-react';
...
const BuyerLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { path: '/buyer/rfq', label: 'RFQs', icon: FileText },
    { path: '/buyer/orders', label: 'Orders', icon: Package },
    { path: '/buyer/documents', label: 'Documents', icon: FileText },
      ...
      <div style={{ marginLeft: sidebarWidth, flex: 1, transition: 'margin-left 0.3s', minHeight: '100vh' }}>
        <div style={{ background: 'white', padding: '16px 32px', borderBottom: '1px solid #e8ecf0', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ width: '36px', height: '36px', background: '#2d3561', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: '600' }}>{user?.name?.charAt(0) || 'U'}</div>
            <div><div style={{ fontWeight: '500', fontSize: '14px' }}>{user?.name || 'User'}</div><div style={{ fontSize: '11px', color: '#999' }}>{user?.email || ''}</div></div>
          </div>
        </div>
