...
import {
  ...
  Download, Trash2, Plus, X, PenTool, Eye, User, Mail, Phone, MapPin
} from 'lucide-react';
...
const SupplierOrderDetails: React.FC = () => {
  ...
  const [statusFileDragging, setStatusFileDragging] = useState(false);
  ...
  const handleSign = async () => {
    ...
    setSigning(true);
    try {
      await realApi.signPO(id!, signatureData, 'supplier', typedName, signedDate);
      await loadData();
  ...
  const handleStatusFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files?.[0]) setStatusFile(e.target.files[0]);
  };

  const handleStatusDrop = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    setStatusFileDragging(false);
    if (e.dataTransfer.files?.[0]) setStatusFile(e.dataTransfer.files[0]);
  };

  const handleStatusDragOver = (e: React.DragEvent<HTMLDivElement>) => {
    e.preventDefault();
    setStatusFileDragging(true);
  };

  const handleStatusDragLeave = () => setStatusFileDragging(false);

  const confirmStatusUpdate = async () => {
    if (!nextStatus) return;
    setUpdating(true);
    try {
      await realApi.updatePOStatus(id!, nextStatus, statusNotes);
      if (statusFile) {
        await realApi.uploadDocument(statusFile, id!, 'po');
      }
      await loadData();
      setShowStatusModal(false);
      alert(`Order status updated to ${nextStatus} successfully!`);
    } catch (err) { alert('Failed to update status'); } finally { setUpdating(false); }
  };
  ...
  const downloadPDF = async () => {
    const element = document.getElementById('po-content-supplier');
    if (!element) return;
  ...
  const handleView = (filename: string) => window.open(getFileUrl(filename), '_blank');
  ...
  const nextStatusFlow = currentIndex >= 0 && currentIndex < statusFlow.length - 1 ? statusFlow[currentIndex + 1] : null;
  const canUpdate = nextStatusFlow && po.status !== 'delivered';
    ...
    <div className="page-container">
      ...
      <div className="split-layout">
        ...
        <div className="po-column">
          <div id="po-content-supplier" className="po-content">
            ...
            <div className="signatures">
              <div><strong>Buyer Signature:</strong>{po.buyer_signature ? <img src={po.buyer_signature} alt="buyer" /> : <span>Not signed</span>}</div>
              <div>
          ...
          {canUpdate && nextStatusFlow && (
            <button onClick={() => updateStatus(nextStatusFlow)} disabled={updating} className="status-update-btn">
              <Truck size={18} /> Mark as {nextStatusFlow}
            </button>
          )}
        </div>
                  ...
                  documents.map((doc: any) => (
                    <div key={doc.id} className="doc-item">
                      ...
                      <div className="doc-actions">
                        <button onClick={() => handleView(doc.filename)}><Eye size={14} /></button>
                        <button onClick={() => handleDownload(doc.filename, doc.original_name)}><Download size={14} /></button>
                        <button onClick={() => handleDeleteDocument(doc.id)}><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))
                )}
      ...
      {showStatusModal && nextStatus && (
        <div className="modal-overlay" onClick={() => setShowStatusModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h3>Mark as {nextStatus}</h3><button onClick={() => setShowStatusModal(false)}><X size={24} /></button></div>
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontWeight: '500', marginBottom: '6px' }}>Notes (optional)</label>
              <textarea value={statusNotes} onChange={(e) => setStatusNotes(e.target.value)} placeholder="Add any notes..." rows={3} style={{ width: '100%', padding: '10px', border: '1px solid #e2e8f0', borderRadius: '12px', fontFamily: 'inherit' }} />
            </div>
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', fontWeight: '500', marginBottom: '6px' }}>Attach Document</label>
              <div className={`drop-zone ${statusFileDragging ? 'dragging' : ''}`} onDrop={handleStatusDrop} onDragOver={handleStatusDragOver} onDragLeave={handleStatusDragLeave} onClick={() => document.getElementById('status-file-input')?.click()} style={{ padding: '20px' }}>
                <Upload size={32} />
                <p>{statusFile ? statusFile.name : 'Click or drag to upload'}</p>
                <p className="sub">{statusFile ? `${(statusFile.size / 1024).toFixed(1)} KB` : 'Supports all formats'}</p>
                <input id="status-file-input" type="file" onChange={handleStatusFileChange} style={{ display: 'none' }} accept="*/*" />
              </div>
            </div>
            <div className="modal-actions">
              <button onClick={() => setShowStatusModal(false)} className="cancel-btn">Cancel</button>
              <button onClick={confirmStatusUpdate} disabled={updating} className="confirm-btn">{updating ? 'Updating...' : 'Confirm'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        .status-update-btn { width: 100%; margin-top: 16px; padding: 12px; background: #3b82f6; color: white; border: none; border-radius: 40px; cursor: pointer; font-weight: 500; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.2s; }
        .status-update-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(59,130,246,0.3); }
