...
import { LayoutDashboard, FolderOpen, FileText, Truck, Users, User, LogOut, Menu, X, Bell, ChevronDown } from 'lucide-react';
...
const BuyerLayout: React.FC = () => {
  ...
  // 🔥 Handle notification click – mark as read, navigate, close dropdown
  ...
  const navItems = [
    ...
    { path: '/buyer/projects', label: 'Projects', icon: FolderOpen },
    { path: '/buyer/orders', label: 'Orders', icon: Truck },
    { path: '/buyer/suppliers', label: 'Supplier Networks', icon: Users },
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#f8fafc' }}>
      ...
      <div style={{
        ...
        <div style={{ padding: sidebarOpen ? '24px 20px' : '20px 0', borderBottom: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: sidebarOpen ? 'space-between' : 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => navigate('/buyer/dashboard')}>
            {sidebarOpen && <span style={{ fontSize: '18px', fontWeight: '700', color: 'white', letterSpacing: '-0.5px' }}>AXO Networks Buyer</span>}
          </div>
        ...
        <div style={{ padding: '12px 32px', borderBottom: '1px solid #eef2f6', display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '24px', position: 'sticky', top: 0, zIndex: 90, backdropFilter: 'blur(8px)', background: 'rgba(255,255,255,0.8)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <div style={{ fontWeight: '600', fontSize: '14px' }}>{user.name || 'Buyer'}</div>
